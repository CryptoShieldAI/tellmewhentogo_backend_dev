...
class BybitApi():
    ...
    def __init__(self, market, api_key = 'DRRDAFDYACYXVIKTAB', secret = 'ILCFYXXKINOTIWUHJGEWNJRSRZHYZLEOXLRL', test=False):
    ...
    def _on_open(self):
        ...
        self.ws.send(json.dumps(
            {
                'op': 'subscribe',  
                'args': ['tickers.' + str(self.market.symbol)]
            }
        ))
    ...
    def getKline(self, resolution, _from, to):
        ...
        try:
            ...
            res = json.loads(requests.get(f"{self.url}/v5/market/kline?{req_params}").text)
